...
use Pum\Bundle\CoreBundle\Security\Authorization\Voter\InstanceVoter;
...
class InstanceVoterTest extends \PHPUnit_Framework_TestCase
{
    private $voter;
    private $user;
    private $project;
    ...
    public function setUp()
    {
        ...
        $this->voter = new InstanceVoter($pumContext);
        $this->user = new User();
        ...
        //The manager user which has PUM_INSTANCE_LIST permission on FooObject#1
        $managerGroup = new Group('Managers');
        $managerUser = new User();
        $managerUser->addGroup($managerGroup);
        $listPermission = new Permission();
        $listPermission
            ->setAttribute('PUM_INSTANCE_LIST')
            ->setProject($this->project)
            ->setBeam($this->beam)
            ->setObject($this->object)
            ->setInstance($this->object->getId())
    ...
    {
        $this->assertSame(VoterInterface::ACCESS_ABSTAIN, $this->voter->vote($this->freshToken, new \stdClass(), ['PUM_INSTANCE_LIST']));
    }
    ...
    {
        $this->voter->vote($this->freshToken, $this->object, ['PUM_INSTANCE_LIST', 'PUM_INSTANCE_VIEW']);
    }

    public function testVoterDenyWhenUserIsNotAuthenticated()
    {
        $this->assertSame(VoterInterface::ACCESS_DENIED, $this->voter->vote($this->anonymousToken, $this->object, ['PUM_INSTANCE_LIST']));
    }

    public function testVoterDenyWhenUserHasNoGroup()
    {
        $this->assertSame(VoterInterface::ACCESS_DENIED, $this->voter->vote($this->lostToken, $this->object, ['PUM_INSTANCE_LIST']));
    }

    public function testVoterDenyWhenUserDoesNotHavePermission()
    {
        $this->assertSame(VoterInterface::ACCESS_DENIED, $this->voter->vote($this->freshToken, $this->object, ['PUM_INSTANCE_LIST']));
    }

    //User is admin but that does not give him the permission on the object instances
    public function testVoterDenyWhenUserIsAdmin()
    {
        $this->assertSame(VoterInterface::ACCESS_DENIED, $this->voter->vote($this->adminToken, $this->object, ['PUM_INSTANCE_LIST']));
    }

    public function testVoterGrantsWhenUserHasPermission()
    {
        $this->assertSame(VoterInterface::ACCESS_GRANTED, $this->voter->vote($this->managerToken, $this->object, ['PUM_INSTANCE_LIST']));
    }

    public function testVoterDenyWhenUserDoesNotHaveGivenPermission()
    {
        $this->assertSame(VoterInterface::ACCESS_DENIED, $this->voter->vote($this->managerToken, $this->object, ['PUM_INSTANCE_DELETE']));
    }
}
