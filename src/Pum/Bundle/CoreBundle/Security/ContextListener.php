...
use Pum\Bundle\CoreBundle\Security\PumUserProvider;
use Pum\Core\Extension\Security\PumUserInterface;
...
/**
 * This class is heavily duplicated from Symfony\Component\Security\Http\Firewall\ContextListener
 *
 * It only adds a few logic to serialize the username only when it's a PumUser.
 *
 * New lines are marked with "NEW" in this file
 */
class ContextListener implements ListenerInterface
{
    ...
    public function handle(GetResponseEvent $event)
    {
        ...
        // NEW!!!
        if (0 === strpos($token, 'PUM:')) {
            $token = $this->recreateToken(substr($token, 4));
        } else {
    ...
    public function onKernelResponse(FilterResponseEvent $event)
    {
        ...
        if ((null === $token = $this->context->getToken()) || ($token instanceof AnonymousToken)) {
            ...
            // NEW!!!
            if ($token->getUser() instanceof PumUserInterface) {
                $token = 'PUM:'.$token->getUser()->getUsername();
            } else {
                $token = serialize($token);
            }
            // END NEW!!!
            $session->set('_security_'.$this->contextKey, $token);
        }
    }
    ...
    private function recreateToken($username)
    {
        foreach ($this->userProviders as $provider) {
            if ($provider instanceof PumUserProvider) {
                $user = $provider->loadUserByUsername($username);
                $token = new UsernamePasswordToken($user, '', md5(microtime()), $user->getRoles());
